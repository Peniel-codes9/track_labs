...
port 3000" — the server won't open the port until the database is actually
connected.
...
Controller: reads `req`, picks status codes, shapes responses. Model: owns
every data rule (required fields, ranges, unique isbn) and never touches
`req`/`res`.

**2. Who owns "can't delete a book while copies are on loan"?**
The model — it's the only layer every delete path (route, script, job)
always passes through.

**3. Why refuse a client-supplied `copiesAvailable` on PATCH?**
It should only change from real borrows/returns, not a client's say-so —
so it's left off the PATCH whitelist entirely.

## Evidence (fill in after running against your own Atlas cluster)

```
curl -i -X POST http://localhost:3000/api/books -H "Content-Type: application/json" -d '{"title":"Things Fall Apart","author":"Chinua Achebe","isbn":"9780385474542","genre":"fiction","publishedYear":1958,"copiesTotal":3}'
```
201 + Location header:
```
...
```
curl -i -X POST http://localhost:3000/api/books -H "Content-Type: application/json" -d '{"author":"Chinua Achebe","isbn":"9780385474542","publishedYear":1958}'
```
400, missing title:
```
...
```
curl -i -X POST http://localhost:3000/api/books -H "Content-Type: application/json" -d '{"title":"Second Copy","author":"Someone","isbn":"9780385474542","publishedYear":2000}'
```
409, duplicate isbn:
```
...
```
curl -i "http://localhost:3000/api/books?author=Ama+Ata+Aidoo"
```
...
```
curl -i http://localhost:3000/api/books/64f000000000000000000000
```
...
```
curl -i http://localhost:3000/api/books/not-an-id
```
400, bad id format:
```
...
```
curl -i http://localhost:3000/api/books/<real-id>
curl -i -X PATCH http://localhost:3000/api/books/<real-id> -H "Content-Type: application/json" -d '{"publishedYear":1959}'
curl -i http://localhost:3000/api/books/<real-id>
```
...
```
curl -i -X PATCH http://localhost:3000/api/books/<real-id> -H "Content-Type: application/json" -d '{"publishedYear":1200}'
```
...
```
curl -i -X DELETE http://localhost:3000/api/books/<real-id>
curl -i -X DELETE http://localhost:3000/api/books/<real-id>
```
204 then 404:
```
